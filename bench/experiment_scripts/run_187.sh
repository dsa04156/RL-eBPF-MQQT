#!/bin/bash
# 완전 자동화 실험 런 187 - LHS 샘플 #187
# RTT: 3990ms, 제어: 4Hz/8batch
# 네트워크: light (지연: 38ms, 손실: 0.8%)
...
echo "[설정] 실험 설정:"
echo "  - RTT: 3990ms (Hi) / 3029ms (Lo)"
echo "  - 제어: 4Hz, 8batch"
echo "  - Publisher: 24개, 48msg/s"
echo "  - Subscriber: 11개"
echo "  - 지속시간: 158초"
echo "  - 네트워크: light (38ms, 0.8% 손실)"
...
cat > ~/mqtt-ebpf-edge/results/generated/$RUN_ID/metadata.json << EOF
{
    ...
    "lhs_sample_id": 187,
    "network_config": {'scenario': 'light', 'delay_ms': 38, 'loss_pct': 0.8074205817242277, 'bandwidth_mbps': 70},
    "workload_config": {'RATE': 48, 'BATCH': 10, 'QOS': 0, 'PAYLOAD_BYTES': 1024, 'PUB_COUNT': 24, 'SUB_COUNT': 11, 'DURATION': 158},
    "eda_config": {'HI_RTT_US': 3990674, 'LO_RTT_US': 3029796, 'T_HOLD_ON': 3.23, 'T_HOLD_OFF': 6.06, 'INTERVAL_S': 4.04, 'CONTROL_MIN_SEC': 4.04, 'CTRL_RATE': 4, 'CTRL_BATCH': 8, 'EWMA_ALPHA': 0.358, 'TH_RETRANS': 2, 'TH_SNDBUF': 572644, 'TH_RCVBUF': 572644, 'SND_RATIO_HI': 0.906, 'SND_RATIO_LO': 0.635, 'USE_ADAPTIVE': 0, 'SAMPLE_ID': 187, 'LHS_COORDS': [0.7961, 0.7953, 0.1714, 0.8128, 0.6456, 0.5824, 0.4813, 0.8259, 0.0313]},
    "infrastructure": {
