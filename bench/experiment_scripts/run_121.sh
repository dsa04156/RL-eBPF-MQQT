#!/bin/bash
# 완전 자동화 실험 런 121 - LHS 샘플 #121
# RTT: 4894ms, 제어: 8Hz/3batch
# 네트워크: burst (지연: 77ms, 손실: 1.9%)
...
echo "[설정] 실험 설정:"
echo "  - RTT: 4894ms (Hi) / 3895ms (Lo)"
echo "  - 제어: 8Hz, 3batch"
echo "  - Publisher: 17개, 48msg/s"
echo "  - Subscriber: 8개"
echo "  - 지속시간: 194초"
echo "  - 네트워크: burst (77ms, 1.9% 손실)"
...
cat > ~/mqtt-ebpf-edge/results/generated/$RUN_ID/metadata.json << EOF
{
    ...
    "lhs_sample_id": 121,
    "network_config": {'scenario': 'burst', 'delay_ms': 77, 'loss_pct': 1.885051954800456, 'bandwidth_mbps': 17},
    "workload_config": {'RATE': 48, 'BATCH': 1, 'QOS': 2, 'PAYLOAD_BYTES': 256, 'PUB_COUNT': 17, 'SUB_COUNT': 8, 'DURATION': 194},
    "eda_config": {'HI_RTT_US': 4894955, 'LO_RTT_US': 3895189, 'T_HOLD_ON': 3.12, 'T_HOLD_OFF': 5.85, 'INTERVAL_S': 3.9, 'CONTROL_MIN_SEC': 3.9, 'CTRL_RATE': 8, 'CTRL_BATCH': 3, 'EWMA_ALPHA': 0.216, 'TH_RETRANS': 3, 'TH_SNDBUF': 806517, 'TH_RCVBUF': 806517, 'SND_RATIO_HI': 0.74, 'SND_RATIO_LO': 0.518, 'USE_ADAPTIVE': 0, 'SAMPLE_ID': 121, 'LHS_COORDS': [0.9788, 0.7654, 0.4154, 0.3308, 0.2901, 0.7863, 0.7362, 0.1612, 0.1976]},
    "infrastructure": {
