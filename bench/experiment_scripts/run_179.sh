#!/bin/bash
# 완전 자동화 실험 런 179 - LHS 샘플 #179
# RTT: 255ms, 제어: 9Hz/7batch
# 네트워크: light (지연: 34ms, 손실: 0.7%)
...
echo "[설정] 실험 설정:"
echo "  - RTT: 255ms (Hi) / 155ms (Lo)"
echo "  - 제어: 9Hz, 7batch"
echo "  - Publisher: 22개, 48msg/s"
echo "  - Subscriber: 9개"
echo "  - 지속시간: 198초"
echo "  - 네트워크: light (34ms, 0.7% 손실)"
...
cat > ~/mqtt-ebpf-edge/results/generated/$RUN_ID/metadata.json << EOF
{
    ...
    "lhs_sample_id": 179,
    "network_config": {'scenario': 'light', 'delay_ms': 34, 'loss_pct': 0.6857093917742235, 'bandwidth_mbps': 97},
    "workload_config": {'RATE': 48, 'BATCH': 2, 'QOS': 0, 'PAYLOAD_BYTES': 1024, 'PUB_COUNT': 22, 'SUB_COUNT': 9, 'DURATION': 198},
    "eda_config": {'HI_RTT_US': 255976, 'LO_RTT_US': 155716, 'T_HOLD_ON': 3.55, 'T_HOLD_OFF': 6.66, 'INTERVAL_S': 4.44, 'CONTROL_MIN_SEC': 4.44, 'CTRL_RATE': 9, 'CTRL_BATCH': 7, 'EWMA_ALPHA': 0.288, 'TH_RETRANS': 2, 'TH_SNDBUF': 739239, 'TH_RCVBUF': 739239, 'SND_RATIO_HI': 0.812, 'SND_RATIO_LO': 0.568, 'USE_ADAPTIVE': 0, 'SAMPLE_ID': 179, 'LHS_COORDS': [0.0416, 0.8809, 0.4238, 0.7636, 0.4702, 0.7387, 0.6629, 0.4478, 0.3337]},
    "infrastructure": {
