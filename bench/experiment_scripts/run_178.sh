#!/bin/bash
# 완전 자동화 실험 런 178 - LHS 샘플 #178
# RTT: 3010ms, 제어: 4Hz/3batch
# 네트워크: moderate (지연: 148ms, 손실: 1.8%)
...
echo "[설정] 실험 설정:"
echo "  - RTT: 3010ms (Hi) / 2166ms (Lo)"
echo "  - 제어: 4Hz, 3batch"
echo "  - Publisher: 15개, 90msg/s"
echo "  - Subscriber: 12개"
echo "  - 지속시간: 181초"
echo "  - 네트워크: moderate (148ms, 1.8% 손실)"
...
cat > ~/mqtt-ebpf-edge/results/generated/$RUN_ID/metadata.json << EOF
{
    ...
    "lhs_sample_id": 178,
    "network_config": {'scenario': 'moderate', 'delay_ms': 148, 'loss_pct': 1.803223612872799, 'bandwidth_mbps': 28},
    "workload_config": {'RATE': 90, 'BATCH': 2, 'QOS': 2, 'PAYLOAD_BYTES': 128, 'PUB_COUNT': 15, 'SUB_COUNT': 12, 'DURATION': 181},
    "eda_config": {'HI_RTT_US': 3010028, 'LO_RTT_US': 2166008, 'T_HOLD_ON': 0.3, 'T_HOLD_OFF': 0.56, 'INTERVAL_S': 0.37, 'CONTROL_MIN_SEC': 0.37, 'CTRL_RATE': 4, 'CTRL_BATCH': 3, 'EWMA_ALPHA': 0.13, 'TH_RETRANS': 2, 'TH_SNDBUF': 776697, 'TH_RCVBUF': 776697, 'SND_RATIO_HI': 0.721, 'SND_RATIO_LO': 0.505, 'USE_ADAPTIVE': 1, 'SAMPLE_ID': 178, 'LHS_COORDS': [0.598, 0.0158, 0.1666, 0.269, 0.0741, 0.6759, 0.7037, 0.0854, 0.9593]},
    "infrastructure": {
