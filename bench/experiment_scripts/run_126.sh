#!/bin/bash
# 완전 자동화 실험 런 126 - LHS 샘플 #126
# RTT: 1496ms, 제어: 9Hz/8batch
# 네트워크: moderate (지연: 92ms, 손실: 1.9%)
...
echo "[설정] 실험 설정:"
echo "  - RTT: 1496ms (Hi) / 985ms (Lo)"
echo "  - 제어: 9Hz, 8batch"
echo "  - Publisher: 19개, 38msg/s"
echo "  - Subscriber: 8개"
echo "  - 지속시간: 183초"
echo "  - 네트워크: moderate (92ms, 1.9% 손실)"
...
cat > ~/mqtt-ebpf-edge/results/generated/$RUN_ID/metadata.json << EOF
{
    ...
    "lhs_sample_id": 126,
    "network_config": {'scenario': 'moderate', 'delay_ms': 92, 'loss_pct': 1.8666722212455842, 'bandwidth_mbps': 49},
    "workload_config": {'RATE': 38, 'BATCH': 1, 'QOS': 2, 'PAYLOAD_BYTES': 256, 'PUB_COUNT': 19, 'SUB_COUNT': 8, 'DURATION': 183},
    "eda_config": {'HI_RTT_US': 1496583, 'LO_RTT_US': 985422, 'T_HOLD_ON': 3.71, 'T_HOLD_OFF': 6.95, 'INTERVAL_S': 4.64, 'CONTROL_MIN_SEC': 4.64, 'CTRL_RATE': 9, 'CTRL_BATCH': 8, 'EWMA_ALPHA': 0.122, 'TH_RETRANS': 0, 'TH_SNDBUF': 514054, 'TH_RCVBUF': 514054, 'SND_RATIO_HI': 0.945, 'SND_RATIO_LO': 0.661, 'USE_ADAPTIVE': 1, 'SAMPLE_ID': 126, 'LHS_COORDS': [0.2922, 0.9225, 0.4716, 0.7917, 0.055, 0.205, 0.4174, 0.9795, 0.8414]},
    "infrastructure": {
