#!/bin/bash
# 완전 자동화 실험 런 186 - LHS 샘플 #186
# RTT: 1885ms, 제어: 8Hz/2batch
# 네트워크: burst (지연: 99ms, 손실: 0.6%)
...
echo "[설정] 실험 설정:"
echo "  - RTT: 1885ms (Hi) / 1270ms (Lo)"
echo "  - 제어: 8Hz, 2batch"
echo "  - Publisher: 24개, 51msg/s"
echo "  - Subscriber: 9개"
echo "  - 지속시간: 187초"
echo "  - 네트워크: burst (99ms, 0.6% 손실)"
...
cat > ~/mqtt-ebpf-edge/results/generated/$RUN_ID/metadata.json << EOF
{
    ...
    "lhs_sample_id": 186,
    "network_config": {'scenario': 'burst', 'delay_ms': 99, 'loss_pct': 0.5853699107696027, 'bandwidth_mbps': 12},
    "workload_config": {'RATE': 51, 'BATCH': 10, 'QOS': 0, 'PAYLOAD_BYTES': 256, 'PUB_COUNT': 24, 'SUB_COUNT': 9, 'DURATION': 187},
    "eda_config": {'HI_RTT_US': 1885187, 'LO_RTT_US': 1270897, 'T_HOLD_ON': 1.39, 'T_HOLD_OFF': 2.61, 'INTERVAL_S': 1.74, 'CONTROL_MIN_SEC': 1.74, 'CTRL_RATE': 8, 'CTRL_BATCH': 2, 'EWMA_ALPHA': 0.298, 'TH_RETRANS': 1, 'TH_SNDBUF': 421833, 'TH_RCVBUF': 421833, 'SND_RATIO_HI': 0.795, 'SND_RATIO_LO': 0.557, 'USE_ADAPTIVE': 1, 'SAMPLE_ID': 186, 'LHS_COORDS': [0.3707, 0.3058, 0.4139, 0.1547, 0.4957, 0.4485, 0.3169, 0.3812, 0.9544]},
    "infrastructure": {
