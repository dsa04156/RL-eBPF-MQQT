#!/bin/bash
# 완전 자동화 실험 런 129 - LHS 샘플 #129
# RTT: 3735ms, 제어: 11Hz/3batch
# 네트워크: heavy (지연: 246ms, 손실: 6.7%)
...
echo "[설정] 실험 설정:"
echo "  - RTT: 3735ms (Hi) / 2797ms (Lo)"
echo "  - 제어: 11Hz, 3batch"
echo "  - Publisher: 20개, 74msg/s"
echo "  - Subscriber: 12개"
echo "  - 지속시간: 200초"
echo "  - 네트워크: heavy (246ms, 6.7% 손실)"
...
cat > ~/mqtt-ebpf-edge/results/generated/$RUN_ID/metadata.json << EOF
{
    ...
    "lhs_sample_id": 129,
    "network_config": {'scenario': 'heavy', 'delay_ms': 246, 'loss_pct': 6.696789173331581, 'bandwidth_mbps': 16},
    "workload_config": {'RATE': 74, 'BATCH': 1, 'QOS': 1, 'PAYLOAD_BYTES': 256, 'PUB_COUNT': 20, 'SUB_COUNT': 12, 'DURATION': 200},
    "eda_config": {'HI_RTT_US': 3735371, 'LO_RTT_US': 2797434, 'T_HOLD_ON': 2.68, 'T_HOLD_OFF': 5.03, 'INTERVAL_S': 3.35, 'CONTROL_MIN_SEC': 3.35, 'CTRL_RATE': 11, 'CTRL_BATCH': 3, 'EWMA_ALPHA': 0.172, 'TH_RETRANS': 2, 'TH_SNDBUF': 789673, 'TH_RCVBUF': 789673, 'SND_RATIO_HI': 0.914, 'SND_RATIO_LO': 0.64, 'USE_ADAPTIVE': 0, 'SAMPLE_ID': 129, 'LHS_COORDS': [0.7445, 0.6492, 0.5723, 0.3247, 0.1797, 0.525, 0.7178, 0.8561, 0.4248]},
    "infrastructure": {
