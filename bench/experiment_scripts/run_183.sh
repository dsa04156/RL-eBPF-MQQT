#!/bin/bash
# 완전 자동화 실험 런 183 - LHS 샘플 #183
# RTT: 2098ms, 제어: 7Hz/1batch
# 네트워크: heavy (지연: 298ms, 손실: 5.3%)
...
echo "[설정] 실험 설정:"
echo "  - RTT: 2098ms (Hi) / 1432ms (Lo)"
echo "  - 제어: 7Hz, 1batch"
echo "  - Publisher: 21개, 72msg/s"
echo "  - Subscriber: 11개"
echo "  - 지속시간: 155초"
echo "  - 네트워크: heavy (298ms, 5.3% 손실)"
...
cat > ~/mqtt-ebpf-edge/results/generated/$RUN_ID/metadata.json << EOF
{
    ...
    "lhs_sample_id": 183,
    "network_config": {'scenario': 'heavy', 'delay_ms': 298, 'loss_pct': 5.328420536321618, 'bandwidth_mbps': 20},
    "workload_config": {'RATE': 72, 'BATCH': 10, 'QOS': 1, 'PAYLOAD_BYTES': 128, 'PUB_COUNT': 21, 'SUB_COUNT': 11, 'DURATION': 155},
    "eda_config": {'HI_RTT_US': 2098164, 'LO_RTT_US': 1432530, 'T_HOLD_ON': 1.02, 'T_HOLD_OFF': 1.9, 'INTERVAL_S': 1.27, 'CONTROL_MIN_SEC': 1.27, 'CTRL_RATE': 7, 'CTRL_BATCH': 1, 'EWMA_ALPHA': 0.247, 'TH_RETRANS': 0, 'TH_SNDBUF': 235734, 'TH_RCVBUF': 235734, 'SND_RATIO_HI': 0.921, 'SND_RATIO_LO': 0.644, 'USE_ADAPTIVE': 1, 'SAMPLE_ID': 183, 'LHS_COORDS': [0.4138, 0.2064, 0.3562, 0.0195, 0.3677, 0.094, 0.1141, 0.8827, 0.8636]},
    "infrastructure": {
