#!/bin/bash
# 완전 자동화 실험 런 123 - LHS 샘플 #123
# RTT: 1959ms, 제어: 2Hz/9batch
# 네트워크: heavy (지연: 209ms, 손실: 7.5%)
...
echo "[설정] 실험 설정:"
echo "  - RTT: 1959ms (Hi) / 1326ms (Lo)"
echo "  - 제어: 2Hz, 9batch"
echo "  - Publisher: 20개, 55msg/s"
echo "  - Subscriber: 10개"
echo "  - 지속시간: 150초"
echo "  - 네트워크: heavy (209ms, 7.5% 손실)"
...
cat > ~/mqtt-ebpf-edge/results/generated/$RUN_ID/metadata.json << EOF
{
    ...
    "lhs_sample_id": 123,
    "network_config": {'scenario': 'heavy', 'delay_ms': 209, 'loss_pct': 7.480890769785783, 'bandwidth_mbps': 5},
    "workload_config": {'RATE': 55, 'BATCH': 5, 'QOS': 1, 'PAYLOAD_BYTES': 128, 'PUB_COUNT': 20, 'SUB_COUNT': 10, 'DURATION': 150},
    "eda_config": {'HI_RTT_US': 1959200, 'LO_RTT_US': 1326652, 'T_HOLD_ON': 3.34, 'T_HOLD_OFF': 6.26, 'INTERVAL_S': 4.17, 'CONTROL_MIN_SEC': 4.17, 'CTRL_RATE': 2, 'CTRL_BATCH': 9, 'EWMA_ALPHA': 0.272, 'TH_RETRANS': 3, 'TH_SNDBUF': 433213, 'TH_RCVBUF': 433213, 'SND_RATIO_HI': 0.895, 'SND_RATIO_LO': 0.627, 'USE_ADAPTIVE': 0, 'SAMPLE_ID': 123, 'LHS_COORDS': [0.3857, 0.8242, 0.105, 0.9283, 0.431, 0.9858, 0.3293, 0.7811, 0.6894]},
    "infrastructure": {
