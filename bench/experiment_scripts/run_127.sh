#!/bin/bash
# 완전 자동화 실험 런 127 - LHS 샘플 #127
# RTT: 2971ms, 제어: 1Hz/3batch
# 네트워크: light (지연: 19ms, 손실: 0.6%)
...
echo "[설정] 실험 설정:"
echo "  - RTT: 2971ms (Hi) / 2133ms (Lo)"
echo "  - 제어: 1Hz, 3batch"
echo "  - Publisher: 22개, 67msg/s"
echo "  - Subscriber: 11개"
echo "  - 지속시간: 162초"
echo "  - 네트워크: light (19ms, 0.6% 손실)"
...
cat > ~/mqtt-ebpf-edge/results/generated/$RUN_ID/metadata.json << EOF
{
    ...
    "lhs_sample_id": 127,
    "network_config": {'scenario': 'light', 'delay_ms': 19, 'loss_pct': 0.6071899324699456, 'bandwidth_mbps': 79},
    "workload_config": {'RATE': 67, 'BATCH': 5, 'QOS': 2, 'PAYLOAD_BYTES': 128, 'PUB_COUNT': 22, 'SUB_COUNT': 11, 'DURATION': 162},
    "eda_config": {'HI_RTT_US': 2971077, 'LO_RTT_US': 2133303, 'T_HOLD_ON': 2.62, 'T_HOLD_OFF': 4.92, 'INTERVAL_S': 3.28, 'CONTROL_MIN_SEC': 3.28, 'CTRL_RATE': 1, 'CTRL_BATCH': 3, 'EWMA_ALPHA': 0.2, 'TH_RETRANS': 2, 'TH_SNDBUF': 377177, 'TH_RCVBUF': 377177, 'SND_RATIO_HI': 0.892, 'SND_RATIO_LO': 0.624, 'USE_ADAPTIVE': 0, 'SAMPLE_ID': 127, 'LHS_COORDS': [0.5901, 0.6334, 0.0327, 0.2729, 0.2497, 0.6466, 0.2682, 0.7661, 0.1158]},
    "infrastructure": {
