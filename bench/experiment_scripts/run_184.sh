#!/bin/bash
# 완전 자동화 실험 런 184 - LHS 샘플 #184
# RTT: 3940ms, 제어: 6Hz/1batch
# 네트워크: burst (지연: 34ms, 손실: 0.8%)
...
echo "[설정] 실험 설정:"
echo "  - RTT: 3940ms (Hi) / 2983ms (Lo)"
echo "  - 제어: 6Hz, 1batch"
echo "  - Publisher: 19개, 82msg/s"
echo "  - Subscriber: 10개"
echo "  - 지속시간: 155초"
echo "  - 네트워크: burst (34ms, 0.8% 손실)"
...
cat > ~/mqtt-ebpf-edge/results/generated/$RUN_ID/metadata.json << EOF
{
    ...
    "lhs_sample_id": 184,
    "network_config": {'scenario': 'burst', 'delay_ms': 34, 'loss_pct': 0.8390173008726141, 'bandwidth_mbps': 27},
    "workload_config": {'RATE': 82, 'BATCH': 5, 'QOS': 2, 'PAYLOAD_BYTES': 1024, 'PUB_COUNT': 19, 'SUB_COUNT': 10, 'DURATION': 155},
    "eda_config": {'HI_RTT_US': 3940797, 'LO_RTT_US': 2983987, 'T_HOLD_ON': 3.08, 'T_HOLD_OFF': 5.78, 'INTERVAL_S': 3.85, 'CONTROL_MIN_SEC': 3.85, 'CTRL_RATE': 6, 'CTRL_BATCH': 1, 'EWMA_ALPHA': 0.445, 'TH_RETRANS': 1, 'TH_SNDBUF': 190090, 'TH_RCVBUF': 190090, 'SND_RATIO_HI': 0.931, 'SND_RATIO_LO': 0.652, 'USE_ADAPTIVE': 0, 'SAMPLE_ID': 184, 'LHS_COORDS': [0.786, 0.7555, 0.291, 0.0037, 0.8636, 0.4637, 0.0643, 0.9253, 0.708]},
    "infrastructure": {
