#!/bin/bash
# 완전 자동화 실험 런 185 - LHS 샘플 #185
# RTT: 1603ms, 제어: 16Hz/7batch
# 네트워크: burst (지연: 81ms, 손실: 0.2%)
...
echo "[설정] 실험 설정:"
echo "  - RTT: 1603ms (Hi) / 1062ms (Lo)"
echo "  - 제어: 16Hz, 7batch"
echo "  - Publisher: 21개, 62msg/s"
echo "  - Subscriber: 10개"
echo "  - 지속시간: 174초"
echo "  - 네트워크: burst (81ms, 0.2% 손실)"
...
cat > ~/mqtt-ebpf-edge/results/generated/$RUN_ID/metadata.json << EOF
{
    ...
    "lhs_sample_id": 185,
    "network_config": {'scenario': 'burst', 'delay_ms': 81, 'loss_pct': 0.2247126047863146, 'bandwidth_mbps': 23},
    "workload_config": {'RATE': 62, 'BATCH': 2, 'QOS': 2, 'PAYLOAD_BYTES': 1024, 'PUB_COUNT': 21, 'SUB_COUNT': 10, 'DURATION': 174},
    "eda_config": {'HI_RTT_US': 1603688, 'LO_RTT_US': 1062885, 'T_HOLD_ON': 2.22, 'T_HOLD_OFF': 4.16, 'INTERVAL_S': 2.77, 'CONTROL_MIN_SEC': 2.77, 'CTRL_RATE': 16, 'CTRL_BATCH': 7, 'EWMA_ALPHA': 0.154, 'TH_RETRANS': 1, 'TH_SNDBUF': 944058, 'TH_RCVBUF': 944058, 'SND_RATIO_HI': 0.868, 'SND_RATIO_LO': 0.608, 'USE_ADAPTIVE': 0, 'SAMPLE_ID': 185, 'LHS_COORDS': [0.3139, 0.5258, 0.838, 0.7698, 0.1342, 0.2868, 0.8861, 0.6739, 0.1804]},
    "infrastructure": {
