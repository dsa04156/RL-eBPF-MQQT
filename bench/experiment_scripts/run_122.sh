#!/bin/bash
# 완전 자동화 실험 런 122 - LHS 샘플 #122
# RTT: 1127ms, 제어: 2Hz/8batch
# 네트워크: light (지연: 33ms, 손실: 0.1%)
...
echo "[설정] 실험 설정:"
echo "  - RTT: 1127ms (Hi) / 725ms (Lo)"
echo "  - 제어: 2Hz, 8batch"
echo "  - Publisher: 22개, 84msg/s"
echo "  - Subscriber: 8개"
echo "  - 지속시간: 161초"
echo "  - 네트워크: light (33ms, 0.1% 손실)"
...
cat > ~/mqtt-ebpf-edge/results/generated/$RUN_ID/metadata.json << EOF
{
    ...
    "lhs_sample_id": 122,
    "network_config": {'scenario': 'light', 'delay_ms': 33, 'loss_pct': 0.08245640647773, 'bandwidth_mbps': 69},
    "workload_config": {'RATE': 84, 'BATCH': 5, 'QOS': 1, 'PAYLOAD_BYTES': 1024, 'PUB_COUNT': 22, 'SUB_COUNT': 8, 'DURATION': 161},
    "eda_config": {'HI_RTT_US': 1127702, 'LO_RTT_US': 725725, 'T_HOLD_ON': 3.41, 'T_HOLD_OFF': 6.4, 'INTERVAL_S': 4.27, 'CONTROL_MIN_SEC': 4.27, 'CTRL_RATE': 2, 'CTRL_BATCH': 8, 'EWMA_ALPHA': 0.291, 'TH_RETRANS': 1, 'TH_SNDBUF': 1042107, 'TH_RCVBUF': 1042107, 'SND_RATIO_HI': 0.857, 'SND_RATIO_LO': 0.6, 'USE_ADAPTIVE': 0, 'SAMPLE_ID': 122, 'LHS_COORDS': [0.2177, 0.8438, 0.0918, 0.8736, 0.4787, 0.3921, 0.993, 0.6289, 0.4659]},
    "infrastructure": {
