#!/bin/bash
# 완전 자동화 실험 런 181 - LHS 샘플 #181
# RTT: 94ms, 제어: 5Hz/6batch
# 네트워크: heavy (지연: 215ms, 손실: 7.3%)
...
echo "[설정] 실험 설정:"
echo "  - RTT: 94ms (Hi) / 56ms (Lo)"
echo "  - 제어: 5Hz, 6batch"
echo "  - Publisher: 17개, 54msg/s"
echo "  - Subscriber: 8개"
echo "  - 지속시간: 189초"
echo "  - 네트워크: heavy (215ms, 7.3% 손실)"
...
cat > ~/mqtt-ebpf-edge/results/generated/$RUN_ID/metadata.json << EOF
{
    ...
    "lhs_sample_id": 181,
    "network_config": {'scenario': 'heavy', 'delay_ms': 215, 'loss_pct': 7.300109684851397, 'bandwidth_mbps': 17},
    "workload_config": {'RATE': 54, 'BATCH': 2, 'QOS': 1, 'PAYLOAD_BYTES': 1024, 'PUB_COUNT': 17, 'SUB_COUNT': 8, 'DURATION': 189},
    "eda_config": {'HI_RTT_US': 94352, 'LO_RTT_US': 56780, 'T_HOLD_ON': 2.69, 'T_HOLD_OFF': 5.05, 'INTERVAL_S': 3.37, 'CONTROL_MIN_SEC': 3.37, 'CTRL_RATE': 5, 'CTRL_BATCH': 6, 'EWMA_ALPHA': 0.463, 'TH_RETRANS': 3, 'TH_SNDBUF': 349568, 'TH_RCVBUF': 349568, 'SND_RATIO_HI': 0.897, 'SND_RATIO_LO': 0.628, 'USE_ADAPTIVE': 0, 'SAMPLE_ID': 181, 'LHS_COORDS': [0.009, 0.6524, 0.216, 0.6096, 0.908, 0.9613, 0.2381, 0.7891, 0.499]},
    "infrastructure": {
