#!/bin/bash
# 완전 자동화 실험 런 182 - LHS 샘플 #182
# RTT: 4024ms, 제어: 8Hz/5batch
# 네트워크: light (지연: 18ms, 손실: 0.8%)
...
echo "[설정] 실험 설정:"
echo "  - RTT: 4024ms (Hi) / 3061ms (Lo)"
echo "  - 제어: 8Hz, 5batch"
echo "  - Publisher: 20개, 86msg/s"
echo "  - Subscriber: 12개"
echo "  - 지속시간: 192초"
echo "  - 네트워크: light (18ms, 0.8% 손실)"
...
cat > ~/mqtt-ebpf-edge/results/generated/$RUN_ID/metadata.json << EOF
{
    ...
    "lhs_sample_id": 182,
    "network_config": {'scenario': 'light', 'delay_ms': 18, 'loss_pct': 0.7910907739557709, 'bandwidth_mbps': 62},
    "workload_config": {'RATE': 86, 'BATCH': 10, 'QOS': 0, 'PAYLOAD_BYTES': 128, 'PUB_COUNT': 20, 'SUB_COUNT': 12, 'DURATION': 192},
    "eda_config": {'HI_RTT_US': 4024601, 'LO_RTT_US': 3061071, 'T_HOLD_ON': 1.06, 'T_HOLD_OFF': 1.98, 'INTERVAL_S': 1.32, 'CONTROL_MIN_SEC': 1.32, 'CTRL_RATE': 8, 'CTRL_BATCH': 5, 'EWMA_ALPHA': 0.451, 'TH_RETRANS': 0, 'TH_SNDBUF': 175057, 'TH_RCVBUF': 175057, 'SND_RATIO_HI': 0.778, 'SND_RATIO_LO': 0.545, 'USE_ADAPTIVE': 0, 'SAMPLE_ID': 182, 'LHS_COORDS': [0.8029, 0.2176, 0.3907, 0.4468, 0.8765, 0.2251, 0.0479, 0.313, 0.2404]},
    "infrastructure": {
