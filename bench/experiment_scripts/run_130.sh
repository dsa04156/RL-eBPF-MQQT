#!/bin/bash
# 완전 자동화 실험 런 130 - LHS 샘플 #130
# RTT: 825ms, 제어: 1Hz/5batch
# 네트워크: burst (지연: 43ms, 손실: 0.6%)
...
echo "[설정] 실험 설정:"
echo "  - RTT: 825ms (Hi) / 521ms (Lo)"
echo "  - 제어: 1Hz, 5batch"
echo "  - Publisher: 15개, 81msg/s"
echo "  - Subscriber: 9개"
echo "  - 지속시간: 191초"
echo "  - 네트워크: burst (43ms, 0.6% 손실)"
...
cat > ~/mqtt-ebpf-edge/results/generated/$RUN_ID/metadata.json << EOF
{
    ...
    "lhs_sample_id": 130,
    "network_config": {'scenario': 'burst', 'delay_ms': 43, 'loss_pct': 0.582878362460258, 'bandwidth_mbps': 23},
    "workload_config": {'RATE': 81, 'BATCH': 2, 'QOS': 1, 'PAYLOAD_BYTES': 1024, 'PUB_COUNT': 15, 'SUB_COUNT': 9, 'DURATION': 191},
    "eda_config": {'HI_RTT_US': 825856, 'LO_RTT_US': 521402, 'T_HOLD_ON': 3.06, 'T_HOLD_OFF': 5.74, 'INTERVAL_S': 3.83, 'CONTROL_MIN_SEC': 3.83, 'CTRL_RATE': 1, 'CTRL_BATCH': 5, 'EWMA_ALPHA': 0.248, 'TH_RETRANS': 3, 'TH_SNDBUF': 972703, 'TH_RCVBUF': 972703, 'SND_RATIO_HI': 0.866, 'SND_RATIO_LO': 0.607, 'USE_ADAPTIVE': 0, 'SAMPLE_ID': 130, 'LHS_COORDS': [0.1567, 0.7509, 0.0092, 0.5043, 0.3707, 0.9509, 0.9173, 0.6659, 0.0095]},
    "infrastructure": {
