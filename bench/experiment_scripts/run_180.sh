#!/bin/bash
# 완전 자동화 실험 런 180 - LHS 샘플 #180
# RTT: 4287ms, 제어: 7Hz/4batch
# 네트워크: moderate (지연: 146ms, 손실: 1.3%)
...
echo "[설정] 실험 설정:"
echo "  - RTT: 4287ms (Hi) / 3306ms (Lo)"
echo "  - 제어: 7Hz, 4batch"
echo "  - Publisher: 16개, 36msg/s"
echo "  - Subscriber: 12개"
echo "  - 지속시간: 184초"
echo "  - 네트워크: moderate (146ms, 1.3% 손실)"
...
cat > ~/mqtt-ebpf-edge/results/generated/$RUN_ID/metadata.json << EOF
{
    ...
    "lhs_sample_id": 180,
    "network_config": {'scenario': 'moderate', 'delay_ms': 146, 'loss_pct': 1.3073473972211747, 'bandwidth_mbps': 29},
    "workload_config": {'RATE': 36, 'BATCH': 2, 'QOS': 2, 'PAYLOAD_BYTES': 256, 'PUB_COUNT': 16, 'SUB_COUNT': 12, 'DURATION': 184},
    "eda_config": {'HI_RTT_US': 4287799, 'LO_RTT_US': 3306854, 'T_HOLD_ON': 1.18, 'T_HOLD_OFF': 2.22, 'INTERVAL_S': 1.48, 'CONTROL_MIN_SEC': 1.48, 'CTRL_RATE': 7, 'CTRL_BATCH': 4, 'EWMA_ALPHA': 0.37, 'TH_RETRANS': 1, 'TH_SNDBUF': 871629, 'TH_RCVBUF': 871629, 'SND_RATIO_HI': 0.724, 'SND_RATIO_LO': 0.507, 'USE_ADAPTIVE': 1, 'SAMPLE_ID': 180, 'LHS_COORDS': [0.8561, 0.2511, 0.3231, 0.3858, 0.6739, 0.285, 0.8071, 0.095, 0.9898]},
    "infrastructure": {
