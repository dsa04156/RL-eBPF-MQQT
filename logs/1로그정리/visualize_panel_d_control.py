...
# ==================== 그래프 생성 (Dual Y-axis) ====================
fig, ax_action = plt.subplots(figsize=(12, 8))

color_action = '#1f77b4'  # 파랑
color_snd = '#ff7f0e'     # 주황

# 왼쪽 Y축: Action Magnitude
line1 = ax_action.plot(time_smooth, action_smooth, color=color_action, linewidth=4.0, label='액션 크기 |Δrate|', zorder=2)

ax_action.set_xlabel('학습 시간 (초)', fontweight='bold', fontsize=18, fontproperties=FONT_PROP)
ax_action.set_ylabel('액션 크기 |Δrate|', fontweight='bold', fontsize=18, color=color_action, fontproperties=FONT_PROP)
...
line2 = ax_snd.plot(time_smooth, snd_smooth, color=color_snd, linewidth=4.0, alpha=0.7, label='snd_ratio', zorder=2)
line3 = ax_snd.axhline(y=1.0, color='red', linestyle='--', linewidth=2.5, alpha=0.7, label='Overflow (1.0)', zorder=1)
...
# 제목
ax_action.set_title('(d) 제어 효과: 액션 크기 → 버퍼 압력 제어', fontweight='bold', loc='left', pad=15, fontsize=22, fontproperties=FONT_PROP)

# 범례 통합
lines = line1 + line2 + [line3]
labels = [l.get_label() for l in lines]
ax_action.legend(lines, labels, loc='upper right', fontsize=15, framealpha=0.95, edgecolor='gray', prop=FONT_PROP)
...
ax_action.text(0.02, 0.98, textstr, transform=ax_action.transAxes, fontsize=16, 
